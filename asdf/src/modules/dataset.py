...
def load_wifi_dataset():
    wifi = {}
    train_dataset = config.get_train_dataset()
    for dataset in train_dataset:
        with open(os.path.join(dataset, 'wifi.pickle'), 'rb') as fr:
            wifi[os.path.basename(dataset)] = pickle.load(fr)
    return wifi